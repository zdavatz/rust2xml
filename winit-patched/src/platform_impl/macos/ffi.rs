...
// `NSInteger` and `AnyObject` were only referenced by the
// `_CGSSetWindowBackgroundBlurRadius` extern that we removed for App
// Store compliance.  Their imports become unused; leave them out so
// downstream builds with `-Dwarnings` (e.g. our CI) still compile.
//
