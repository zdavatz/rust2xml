...
Rust port of **oddb2xml** — the Ruby tool (~14,261 LOC across 20 modules) that generates Swiss Healthcare Public Domain data files (XML, SQLite, CSV, DAT). All 20 Ruby modules have a corresponding Rust module; the crate builds clean on stable Rust, 58 unit tests + 2 integration tests pass.

Current released version: **v3.1.11** — emits the BAG **Indikationscode** (`XXXXX.NN`) into the actual builder output that ships to the GUI and XML files.  v3.1.9 added the FHIR extraction (`BagItem.indication_codes` / `BagPackage.indication_codes`) but never wired those fields into the `Builder::*_records()` methods, so the codes were parsed into memory and discarded — invisible in the GUI's `products`/`articles`/`limitations` tabs and in `oddb_product.xml` / `oddb_article.xml` / `oddb_limitation.xml`.

This release plumbs them through:
- `Builder::product_nodes` adds `INDIKATIONSCODE` per PRD row, populated from `pkg.indication_codes` (falls back to `item.indication_codes`), comma-joined and deduped via a new `join_indication_codes()` helper.
- `Builder::article_nodes` adds an optional `<INDIKATIONSCODE>` child to ART rows for the BAG/FHIR branch (branches 2-5 — refdata-only, ZurRose-only, Firstbase — pass an empty string and the leaf is suppressed).  `ArtFields` gains an `indikationscodes: String` field; the `into_nodes()` writer emits the leaf only when non-empty.
- `Builder::limitation_records` adds `INDIKATIONSCODE` to every LIM row (one row per package limitation), pulling from the package's codes with item-level fallback.
- New integration test `cyramza_bundle_emits_indikationscode_into_product_article_limitation` runs the CYRAMZA fixture through extractor → builder and asserts `<INDIKATIONSCODE>20403.0...` shows up in product, article and limitation XML output.

In the GUI: the codes now appear as a column called `INDIKATIONSCODE` in the **products**, **articles** and **limitations** tabs (each tab's columns come from `Builder::*_records()` via `sqlite_export`).  Tab cell click-to-expand still works — useful for the comma-joined multi-code packages.
