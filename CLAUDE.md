...
Current released version: **v3.1.15** — GUI first-time download warning + elapsed-time counter, addressing Apple App Review Guideline 4 (Design) feedback that the `Run -e (Extended)` job downloaded ~250 MB without warning the user.  Two GUI-only changes in `src/gui.rs`:
